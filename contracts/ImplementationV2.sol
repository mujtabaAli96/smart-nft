...
contract ImplementationV2 {
  ...
  function buy (
	...
	uint256[3] memory _params1, // token_id, standard, amount 
  ...
  ) public payable
  {
		  ...
		  if( _params2[0]  != true ) {
		  		require( _price == msg.value, "P N C" ); //price not correct
		  }
  ...
  function lazy_mint (
	...
	uint256[] memory _params_two, //chain_id, price_of_one_copy, royalty_percentage, token_id, amount, standard, buy_amount, _split_payment_percentages_from_index_7
  ...
  ) public payable 
  {
	...
	if( _params_three[1] == true ) {
		...
		for( i = 0;  i < _params_two.length; i++ ) {
			if( i >= 7 ) {
				S.con_user_id_details[ _params_one[0] ][ _params_one[1] ][ _params_two[3] ].index_to_split_payment[ i - 6 ].per = _params_two[i];
			}
		}
	...
	buy( 
		...
		[ _params_two[3], _params_two[5], _params_two[6] ],
		[ _params_three[3] ]
	);
