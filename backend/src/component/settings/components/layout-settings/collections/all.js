...
export const AllCollections = () =>{
    ...
    const defaultState = state.collections.all

    const changeState = (property, value) =>{
        const temp = {};
        temp[property] = value;
        const newState = { ...defaultState, ...temp }
        dispatch({ type: "CHANGE_COLLECTIONS_ALL", payload: newState })
    }
        ...
        <>
            ...
            <div className="settings-page__list-settings-single">
                <div className="settings-page__list-settings-single__headings">
                    <h4>{__("Collections view", SLUG)}</h4>
                    <p>
                    {__(
                        "Shows list or grid view depending on choice.",
                        SLUG
                    )}
                    </p>
                </div>
                <div className="settings-page__list-settings-single__controls">
                    <select
                        value={defaultState.view}
                        onChange={(e) => changeState("view", e.target.value)}
                    >
                    <option value="list">{__("List view", SLUG)}</option>
                    <option value="grid">{__("Grid view", SLUG)}</option>
                    </select>
                </div>
            </div>
            <div className="settings-page__list-settings-single">
