...
import { CreateNftIntegratiton } from "../../../backend/src/component/create-nft/create-nft-integrate";
...
window.SMART_NFT_CREATE_NEW_NFT_FORM_RERUN_APP = inisialfn;

//this fn is necessacry to work the script in builder;
//if no need builder support then call the `inisialfn()` directly without `refTimer()` fn;
let INTERVAL = 300;
let timeToMakeNextRequest = 0;
function rafTimer(time) {
  if (timeToMakeNextRequest <= time) {
    timeToMakeNextRequest = time + INTERVAL;
    const widgets = document.querySelector("#smartnft-root");
