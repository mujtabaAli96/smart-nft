...
class SmartNftActivity 
{
	...
	function get_coll_stats_on_time_range() {
		if( 
			!isset( $_POST['startTimeInMS'] ) || 
			empty( $_POST['startTimeInMS'] ) ||
			!isset( $_POST['collId'] ) || 
			empty( $_POST['collId'] ) 
		...
		$start_time_in_second = intval( $_POST['startTimeInMS'] ) / 1000;
		...
		$query = " SELECT * FROM " . $table . " WHERE input_time >= (SELECT FROM_UNIXTIME(". $start_time_in_second . ")) " . 
